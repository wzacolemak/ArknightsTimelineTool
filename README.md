...
需要准备一份可用的新版 Rust 费用尺。费用尺启动后会在本机提供帧数，默认地址为 `ws://127.0.0.1:2606`。

Windows 下推荐使用 [uv](https://docs.astral.sh/uv/) 创建项目环境：

```powershell
git clone https://github.com/wzacolemak/ArknightsTimelineTool.git
cd ArknightsTimelineTool
uv venv
...
打轴器会自动连接费用尺。若没有跟随帧数，先确认费用尺已经运行，并且只启动了一个实例。
...
打包结果位于 `dist/TimelineTool_YYYYMMDD/`，项目根目录还会生成一份 `TimelineTool.exe`。
